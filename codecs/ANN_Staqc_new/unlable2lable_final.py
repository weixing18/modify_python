...
from configs import get_config_u2l
...
class StandoneCode:
    ...
    def get_data(self,path):
        ...
        return text_S1, text_S2, code, queries, labels, ids        #return text_S1, text_S2, code, queries, ids
    ...
    def eval(self, model, path):
        ...
        return acc, f1, recall, precision, loss    #给语料打codemf标签
...
def final_analay_large(path, hnn_path, single_path, save_path):
    ...
    with open(single_path, 'r') as f:
        single = eval(f.read())
    single_ids = [single[i][0] for i in range(len(single))]
...
if __name__ == '__main__':
    ...
    model = eval(conf['model_params']['model_name'])(conf)  # initialize the model,  model== <models.CARLCS_CNN object at 0x7f1d9c2e2cc0>
    ...
    model.params_adjust(dropout1=np.round(0.25, 2), dropout2=np.round(0.25, 2), dropout3=np.round(0.25, 2),
                        dropout4=np.round(0.25, 2), dropout5=np.round(0.25, 2), Regularizer=round(0.0004, 4),
                        num=8, seed=42)
    model.build()
